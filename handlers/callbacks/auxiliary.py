...
from .states import AuxiliaryFSM
...
@router.callback_query(F.data == "workshop_auxiliary")
async def start_auxiliary(callback: CallbackQuery, state: FSMContext):
    await state.clear()
    await state.set_state(AuxiliaryFSM.fullname)
    await state.update_data(workshop_code="auxiliary")
    text = "👨‍🏭 Вспомогательный цех\n\nВведите ваши ФИО:"
    kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🔙 Назад", callback_data="back_to_production")]])
    await callback.message.edit_text(text, reply_markup=kb)
    await callback.answer()

@router.message(AuxiliaryFSM.fullname)
async def process_fullname(message: Message, state: FSMContext, db: Database):
    ...
    if not projects:
        await message.answer("⚠️ Для этого цеха ещё не добавлены проекты. Обратитесь к администратору.")
        await state.clear()
        return
    await message.answer("Выберите проект:", reply_markup=get_projects_keyboard(projects))
    await state.set_state(AuxiliaryFSM.project)

@router.callback_query(AuxiliaryFSM.project, F.data.startswith("project_"))
async def process_project(callback: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(AuxiliaryFSM.efficiency)
    await callback.answer()

@router.callback_query(AuxiliaryFSM.efficiency, F.data.startswith("efficiency_"))
async def process_efficiency(callback: CallbackQuery, state: FSMContext, db: Database):
    ...
    if color == "green":
        ...
        if ADMIN_CHAT_ID:
            ...
            text = f"🔧 Отчёт вспомогательного цеха\n"
            text += f"Дата: {now}\n"
...
@router.callback_query(AuxiliaryFSM.red_reason, F.data == "red_questions")
async def red_questions(callback: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(AuxiliaryFSM.question_desc)
    await callback.answer()

@router.callback_query(AuxiliaryFSM.red_reason, F.data == "red_problems")
async def red_problems(callback: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(AuxiliaryFSM.problem_type)
    await callback.answer()

@router.callback_query(AuxiliaryFSM.problem_type, F.data.startswith("problem_"))
async def process_problem_type(callback: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(AuxiliaryFSM.problem_desc)
    await callback.answer()

@router.message(AuxiliaryFSM.problem_desc, F.text)
async def process_problem_text(message: Message, state: FSMContext):
    ...
    await state.set_state(AuxiliaryFSM.photo_optional)

@router.message(AuxiliaryFSM.question_desc, F.text)
