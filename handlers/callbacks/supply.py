...
async def start_supply(callback: CallbackQuery, state: FSMContext):
    await state.clear()
    await state.set_state(SupplyFSM.fullname)
    await state.update_data(workshop_code="supply")
    text = "🧩 Снабжение\n\nВведите ваши ФИО:"
...
@router.message(SupplyFSM.fullname)
async def process_fullname(message: Message, state: FSMContext, db: Database):
    ...
    await state.set_state(SupplyFSM.project)


@router.callback_query(SupplyFSM.project, F.data.startswith("project_"))
async def process_project(callback: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(SupplyFSM.efficiency)
    await callback.answer()


@router.callback_query(SupplyFSM.efficiency, F.data.startswith("efficiency_"))
async def process_efficiency(callback: CallbackQuery, state: FSMContext, db: Database):
    ...
    if color == "green":
        ...
        await state.set_state(SupplyFSM.problem_desc)
        await callback.answer()


@router.message(SupplyFSM.problem_desc, F.text)
async def process_problem_text(message: Message, state: FSMContext):
    ...
    await state.set_state(SupplyFSM.photo_optional)


@router.callback_query(F.data == "skip_photo")
async def skip_photo(callback: CallbackQuery, state: FSMContext, db: Database):
    current_state = await state.get_state()
    if current_state != SupplyFSM.photo_optional.state:
...
@router.message(SupplyFSM.photo_optional, F.photo)
