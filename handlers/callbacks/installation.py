...
async def start_installation(callback: CallbackQuery, state: FSMContext):
    await state.clear()
    await state.set_state(InstallationFSM.fullname)
    await state.update_data(workshop_code="installation")
    text = "🔧 Монтаж\n\nВведите ваши ФИО:"
...
@router.message(InstallationFSM.fullname)
async def process_fullname(message: Message, state: FSMContext, db: Database):
    ...
    await state.set_state(InstallationFSM.project)


@router.callback_query(InstallationFSM.project, F.data.startswith("project_"))
async def process_project(callback: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(InstallationFSM.efficiency)
    await callback.answer()


@router.callback_query(InstallationFSM.efficiency, F.data.startswith("efficiency_"))
async def process_efficiency(callback: CallbackQuery, state: FSMContext, db: Database):
    ...
    if color == "green":
        ...
        await state.set_state(InstallationFSM.red_reason)
        await callback.answer()


@router.callback_query(InstallationFSM.red_reason, F.data == "red_questions")
async def red_questions(callback: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(InstallationFSM.question_desc)
    await callback.answer()


@router.callback_query(InstallationFSM.red_reason, F.data == "red_problems")
async def red_problems(callback: CallbackQuery, state: FSMContext):
    await state.update_data(report_type="problem")
    await callback.message.edit_text(
        "Выберите тип проблемы:", reply_markup=get_problem_installation_keyboard()
    )
    await state.set_state(InstallationFSM.problem_type)
    await callback.answer()


@router.callback_query(InstallationFSM.problem_type, F.data.startswith("problem_"))
async def process_problem_type(callback: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(InstallationFSM.problem_desc)
    await callback.answer()


@router.message(InstallationFSM.problem_desc, F.text)
async def process_problem_text(message: Message, state: FSMContext):
    ...
    await state.set_state(InstallationFSM.photo_optional)


@router.message(InstallationFSM.question_desc, F.text)
...
async def skip_photo(callback: CallbackQuery, state: FSMContext, db: Database):
    ...
    if current_state != InstallationFSM.photo_optional.state:
...
@router.message(InstallationFSM.photo_optional, F.photo)
