...
from .states import EconomicsFSM
...
async def start_economics(callback: CallbackQuery, state: FSMContext):
    await state.clear()
    await state.set_state(EconomicsFSM.fullname)
    await state.update_data(workshop_code="economics")
    text = "📈 Экономика\n\nВведите ваши ФИО:"
...
@router.message(EconomicsFSM.fullname)
async def process_fullname(message: Message, state: FSMContext, db: Database):
    ...
    await state.set_state(EconomicsFSM.project)


@router.callback_query(EconomicsFSM.project, F.data.startswith("project_"))
async def process_project(callback: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(EconomicsFSM.efficiency)
    await callback.answer()


@router.callback_query(EconomicsFSM.efficiency, F.data.startswith("efficiency_"))
async def process_efficiency(callback: CallbackQuery, state: FSMContext, db: Database):
    ...
    if color == "green":
        ...
        await state.set_state(EconomicsFSM.problem_desc)
        await callback.answer()


@router.message(EconomicsFSM.problem_desc, F.text)
async def process_problem_text(message: Message, state: FSMContext):
    ...
    await state.set_state(EconomicsFSM.photo_optional)


@router.callback_query(F.data == "skip_photo")
async def skip_photo(callback: CallbackQuery, state: FSMContext, db: Database):
    current_state = await state.get_state()
    if current_state != EconomicsFSM.photo_optional.state:
...
@router.message(EconomicsFSM.photo_optional, F.photo)
