...
async def cmd_projects(message: Message):
    if not is_admin(message.from_user.id):
        ...
        return
    await message.answer("🛠 Администрирование проектов\n\nЧто хотите сделать?", reply_markup=get_admin_menu_keyboard())

@router.callback_query(F.data == "admin_add_project")
async def admin_add_project_start(callback: CallbackQuery, state: FSMContext, db: Database):
    ...
    await callback.message.edit_text("Выберите цех для добавления проекта:", reply_markup=keyboard)
    await state.set_state(AdminFSM.waiting_for_workshop_add)
    await callback.answer()

@router.callback_query(AdminFSM.waiting_for_workshop_add, F.data.startswith("admin_workshop_"))
...
async def admin_project_name(message: Message, state: FSMContext, db: Database):
    ...
    if success:
        await message.answer(f"✅ Проект «{project_name}» добавлен для цеха {workshop_code}.")
        await state.clear()
        await message.answer("Главное меню:", reply_markup=get_main_menu_keyboard())
    else:
        await message.answer(f"❌ Проект «{project_name}» уже существует для этого цеха. Попробуйте другое название.", reply_markup=get_cancel_keyboard())


@router.callback_query(F.data == "admin_remove_project")
async def admin_remove_project_start(callback: CallbackQuery, state: FSMContext, db: Database):
    if not is_admin(callback.from_user.id):
        await callback.answer("Нет прав", show_alert=True)
        return
    workshops = await db.get_workshops()
    ...
    await callback.message.edit_text("Выберите цех для удаления проекта:", reply_markup=keyboard)
    await state.set_state(AdminFSM.waiting_for_workshop_remove)
    await callback.answer()

@router.callback_query(AdminFSM.waiting_for_workshop_remove, F.data.startswith("admin_remove_workshop_"))
async def admin_remove_workshop_selected(callback: CallbackQuery, state: FSMContext, db: Database):
    ...
    await callback.message.edit_text("Выберите проект для удаления:", reply_markup=keyboard)
    await state.update_data(workshop_code=workshop_code)
    await state.set_state(AdminFSM.waiting_for_project_remove)
    await callback.answer()

@router.callback_query(AdminFSM.waiting_for_project_remove, F.data.startswith("admin_remove_project_"))
async def admin_remove_project(callback: CallbackQuery, state: FSMContext, db: Database):
