...
class Database:
    ...
    async def _populate_initial_data(cls, db):
        ...
        for name, code in workshops:
            await db.execute(
                "INSERT OR IGNORE INTO workshops (name, code) VALUES (?, ?)",
                (name, code),
            )
        await db.commit()
    ...
    async def _populate_initial_data_prod(cls, db):
        ...
        for name, code in workshops_prod:
            await db.execute(
                "INSERT OR IGNORE INTO workshops_for_prod (name, code) VALUES (?, ?)",
                (name, code),
            )
        await db.commit()
    ...
    async def get_workshops(self) -> List[Workshop]:
        async with aiosqlite.connect(self.db_path) as db:
            ...
            async with db.execute(
                "SELECT id, name, code FROM workshops ORDER BY id"
            ) as cursor:
                rows = await cursor.fetchall()
                return [
                    Workshop(id=row["id"], name=row["name"], code=row["code"])
                    for row in rows
                ]
    ...
    async def get_workshops_prod(self) -> List[Workshop]:
        async with aiosqlite.connect(self.db_path) as db:
            db.row_factory = aiosqlite.Row
            async with db.execute(
                "SELECT id, name, code FROM workshops_for_prod ORDER BY id"
            ) as cursor:
                rows = await cursor.fetchall()
                return [
                    Workshop(id=row["id"], name=row["name"], code=row["code"])
    ...
    async def get_projects(self) -> List[Project]:
        async with aiosqlite.connect(self.db_path) as db:
            db.row_factory = aiosqlite.Row
            async with db.execute(
                "SELECT id, name, code FROM projects ORDER BY id"
            ) as cursor:
                rows = await cursor.fetchall()
                return [
    ...
    async def get_workshop_projects(self, workshop_code: str):
        async with aiosqlite.connect(self.db_path) as db:
            ...
            async with db.execute(
                ...
                rows = await cursor.fetchall()
                return [
                    {"name": row["project_name"], "code": row["project_code"]}
                    for row in rows
                ]
